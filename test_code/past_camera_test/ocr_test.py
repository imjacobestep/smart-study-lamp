...
out_path = 'test_code/past_camera_test/photos/images_crop/2.jpg'
...
details = pytesseract.image_to_data(output3, output_type=pytesseract.Output.DICT)
total_boxes = len(details['text'])

for sequence_number in range(total_boxes):
  if float(details['conf'][sequence_number]) >30:
    (x, y, w, h) = (details['left'][sequence_number], details['top'][sequence_number], details['width'][sequence_number],  details['height'][sequence_number])
    threshold_img = cv2.rectangle(output2, (x, y), (x + w, y + h), (0, 255, 0), 2)
